...
has_outlier_stats <- RregressPkg::ols_calc(x = has_outlier_dt[,.(x,y)], resp_col = "y" )
...
no_outlier_stats <- RregressPkg::ols_calc(x = no_outlier_dt[,.(x,y)], resp_col = "y")
...
has_leverage_stats <- RregressPkg::ols_calc(x = has_leverage_dt[,.(x,y)], resp_col = "y")
...
no_leverage_stats <- RregressPkg::ols_calc(x = no_leverage_dt[,.(x,y)], resp_col = "y")
...
has_outl_lever_stats <- RregressPkg::ols_calc(x = has_outl_lever_dt[,.(x,y)], resp_col = "y")
...
no_outl_lever_stats <- RregressPkg::ols_calc(x = no_outl_lever_dt[,.(x,y)], resp_col = "y")
...
```{r}
ols_stats <- RregressPkg::ols_calc(x = data_dt, resp_col = "y")
```

3. Compute the internal studentized residuals:
```{r}
simple_influence_lst <- RregressPkg::ols_influence_calc(x = data_dt, resp_col = "y")
```
...
internal_influence_lst <- RregressPkg::ols_influence_calc(
  x = dt,
  resp_col = "y",
...
no_outlier_stats <- RregressPkg::ols_calc(x = no_outlier_dt[,.(x,y)], resp_col = "y")
outlier_stats <- RregressPkg::ols_calc(x = outlier_dt[,.(x,y)], resp_col = "y")
...
```{r}
dt <- has_outlier_dt[, .(x, y)]
external_influence_lst <- RregressPkg::ols_influence_calc(
  x = dt, 
  resp_col = "y",
...
```{r}
dt <- has_outlier_dt[, .(x, y)]
diff_influence_lst <- RregressPkg::ols_influence_calc(
  x = dt, 
  resp_col = "y",
...
```{r}
dt <- has_outlier_dt[, .(x, y)]
cook_influence_lst <- RregressPkg::ols_influence_calc(
  x = dt, 
  resp_col = "y",
  influence_meas = "cook"
)
...
```{r}
influence_lst <- RregressPkg::ols_influence_calc(
  x = foot_height_dt,
  resp_col = "foot",
  influence_meas = "cook",
  label_threshold = 3.0,
...
ols_no28_lst <- RregressPkg::ols_calc(x = foot_height_no28_dt, resp_col = "foot")
RplotterPkg::create_table(
...
```{r}
ols_lst <- RregressPkg::ols_calc(x = foot_height_dt, resp_col = "foot")
...
```{r}
ols_lst <- RregressPkg::ols_calc(x = hosp_infect_dt, resp_col = "InfctRsk")
