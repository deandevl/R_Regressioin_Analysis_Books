...
```{r}
pop_data <- therms_pop$df_scaled$data
...
get_sample_means <- function(samples, sample_size, data_pop){
  means <- c()
  for(i in 1:samples){
    means <- c(means, mean(base::sample(x = data_pop, size = sample_size)))
  }
  return(means)
}
simulated_means <- get_sample_means(samples = 100000, sample_size = 10, data_pop = therms_pop$df_scaled$data)
```
...
for(samp_sz in samp_sizes) {
  samples_lst[[paste0("samp size ", samp_sz)]] = data.table(
    ...
    samp_sz = samp_sz, 
    samp_means = lapply(1:10, function(i){x <- mean(sample(therms_pop$df_scaled$data, samp_sz, replace = T))})
  )
}
samples_dt <- data.table::rbindlist(samples_lst, use.names = T)
samples_dt[, `:=` (samp_sz_factor = as.factor(samp_sz), samp_means = as.numeric(samp_means), samp_sz_label = forcats::fct_inorder(paste0("Sample Size: ", samp_sz)))]
```
...
a_plot <- RplotterPkg::create_scatter_plot(
  ...
  pts_size = 2.5,
  pts_line_alpha = 0.5
) + ggplot2::geom_hline(yintercept = mean(therms_pop$df_scaled$data), linetype = "dashed")
a_plot
```
...
```{r}
pop_sd <- sd(therms_pop$df_scaled$data)
...
add_ons <- c(ggplot2::geom_hline(yintercept = mean(therms_pop$df_scaled$data), linetype = "dashed"))
