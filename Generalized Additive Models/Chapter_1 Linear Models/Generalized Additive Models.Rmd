...
We will be using the `gamair::sperm.comp1` data set to illustrate the derivartion.

1. Load the data and define the predictor(**X**) and response(**Y**) matrices:
```{r}
...
X_mt <- as.matrix(X_dt)
Y_mt <- as.matrix(sperm1_dt[, .(count)])
```

2. Perform QR factorization using a defined function:
```{r}
...
2. Decompose **X** and assign **Q**, **Qf** and **R** (Equation 1.5 page 12):
**Qf** is an n x p orthogonal matrix; **R** is a p x p upper trianglar matrix
```{r}
# decompose X
# qr_lst <- base::qr(X_mt)
# 
...
# 
# X_check_mt <- Qf_mt %*% R_mt  # Equation 1.5 page 12
# 
# Q_mt <- base::qr.Q(qr_lst, complete = T) # orthogonal matrix n x n
# Q_check_mt <- t(Q_mt) %*% Q_mt # should get identity matrix
```

3. Compute the t(**Q**) * **Y** and assign vectors **f** and **r**:
```{r}
...
#sse_alt <- (t(resid_mt) %*% resid_mt)[1,1]
```
...
2. Compute covariance matrix of $\hat{\sigma}$ (Equation 1.7 page 13):
```{r}
var_cov_beta <- R_inv_mt %*% t(R_inv_mt) * sigma_sq # p x p
```

3. Compute the standard error of $\hat{\sigma}$ (page 14)
```{r}
...
```{r}
sperm_model_1_dt <- RregressPkg::ols_calc(
  df = sperm1_dt[,!c("subject")],
  formula_obj = count ~ time_ipc + prop_partner
)
RplotterPkg::create_table(
  x = sperm_model_1_dt$coef_df
)
```

```{r}
model_1_lm <- lm(count ~ time_ipc + prop_partner, data = sperm1_dt)
summary(model_1_lm)
```
