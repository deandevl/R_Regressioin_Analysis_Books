...
1. Read the data:
```{r}
gpa1_dt <- data.table::setDT(wooldridge::gpa1)
gpa1_dt <- gpa1_dt[, .(colGPA, hsGPA, ACT)]
```
...
1. Read the data:
```{r}
wage1_dt <- data.table::setDT(wooldridge::wage1)
wage1_dt <- wage1_dt[, .(wage, educ, exper, tenure)]
```
...
1. Read the data:
```{r}
k401k_dt <- data.table::setDT(wooldridge::k401k)
k401k_dt <- k401k_dt[, .(prate, mrate, age)]
```
...
1. Read the data:
```{r}
crime1_dt <- data.table::setDT(wooldridge::crime1)
crime1_dt <- crime1_dt[, .(narr86, pcnv, ptime86, qemp86, avgsen)]
```
...
```{r}
gpa1_dt <- data.table::setDT(wooldridge::gpa1)
gpa1_dt <- gpa1_dt[, .(colGPA, ACT, hsGPA)]
```
...
```{r}
wage_model <- lm(log(wage) ~ educ + exper + tenure, data = wage1_dt)
summary(wage_model)
```
