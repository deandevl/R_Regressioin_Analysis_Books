...
<center>$y = \hat{\beta_{0}} + \hat{\beta_{1}}x + \mu$<span style="float:right;">[2.1]</span></center> 

According to `(Wooldridge, Section 2.2, Equations 2.17, 2.19, page 26)` we can estimate the OLS regression line with:
<center>$\hat{\beta_{0}} = \bar{y} - \hat{\beta_{1}}\bar{x}$<span style="float:right;">[2.2]</span></center>
<center>$\hat{\beta_{1}} = \frac{Cov(x,y)}{Var(x)}$<span style="float:right;">[2.3]</span></center>

<div class="task">Task:  From the `wooldridge::ceosal1` data set involving CEO Salary (*salary*) and Return on Equity (*roe*) calculate the statistics for estimating the population parameters $\hat{\beta_{0}}$ and $\hat{\beta_{1}}$ for the *roe* variable to estimate *salary*.</div>

1. Review the *roe* and *salary* variables from the data set:
```{r}
...
```{r}
cov_roe_salary <- stats::cov(ceosal1$roe, ceosal1$salary)
var_roe <- stats::var(ceosal1$roe)
...
3. From `(Wooldridge, Equation 2.17 page 26)` estimate $\hat{\beta}_0$ where we need the means for *roe* and *salary* and $\hat{\beta}_1$:
```{r}
b0_hat <- mean(ceosal1$salary) - b1_hat * mean(ceosal1$roe)
```

 $\hat{\beta}_0 = \bar{y} - \hat{\beta_{1}}\bar{x} =$ `r b0_hat`

<div class="task">Task: Use `stats::lm()` to repeat the above estimate of the coefficients $\hat{\beta_{0}}$ and $\hat{\beta_{1}}$, regressing *salary* on *roe* and plot the linear fit.</div> 

1. Compute the coefficients via `stats::lm()`:
```{r}
roe_salary_lm <- stats::lm(salary ~ roe, data = ceosal1)
```
...
df <- data.frame(
  roe = ceosal1$roe,
  salary = roe_salary_lm$model$salary
)
RplotterPkg::create_scatter_plot(
  df = df,
  aes_x = "roe",
  aes_y = "salary",
  title = "Salary Regressed on Return on Equity (ROE)",
  subtitle = "data set - wooldridge::ceosal1",
...
<div class="task">Task: Find the relationship between wage and education `(Wooldridge, Example 2.4, page 30)` from the data set *wooldridge::wage1*.</div>  

1. Review the *wage* and *educ* variables from the data set:
```{r}
...
```{r}
wage_educ_lm <- stats::lm(wage ~ educ, data = wage1)
```
...
df <- data.frame(
  educ = wage1$educ,
  wage = wage_educ_lm$model$wage
)
RplotterPkg::create_scatter_plot(
  df = df,
  aes_x = "educ",
  aes_y = "wage",
  title = "Wage Regressed on Education",
  subtitle = "data set - wooldridge::wage1",
  x_title = "Education",
  y_title = "Wage (%)",
  rot_y_tic_label = TRUE,
  x_limits = c(0,18),
...
1. Sum and sample mean of the residuals: $\sum_{i=1}^n \hat{\mu_{i}} = 0 \Rightarrow \bar{\hat{\mu}}_{i} = 0$<span style="float:right;">[2.7]</span>

2. Sample covariance between the regressors and the OLS residuals: $\sum_{i=1}^n x_{i}\hat{\mu}_{i} = 0$   $\Rightarrow$   $Cov(x_{i}, \hat{\mu}_{i}) = 0$<span style="float:right;">[2.8]</span>

3. The mean is always on the line: $\bar{y} = \hat{\beta}_0 + \hat{\beta}_1 \cdot \bar{x}$<span style="float:right;">[2.9]</span>

<div class="task">Task: Take the above wage and education data set and confirm the three properties of OLS.</div>  
...
2. Property 2 -- The sample covariance between the regressors and the OLS residuals is zero. $Cov(x_{i},\hat{\mu}_{i}) = 0$
```{r}
cov_x_mu <- stats::cov(wage1$educ, mu_hat)
```
...
y_mean_lm <- lm_coef[1] + lm_coef[2] * mean(wage1$educ)
```
...
<div class="task">Task: Using the above data of *salary* on *roe* compute the $R^2$ or coefficient of determination three different ways where `(Wooldridge, Equations 2.33,2.34,2.35 page 34)` defines:.</div>  

<br><center>Total sum of the squares (SST) = $\sum_{i=1}^n(y_{i} - \bar{y})^2 = (n - 1) \cdot Var(y)$<span style="float:right;">[2.10]</span></center>

<br><center>Explained sum of the squares (SSE) = $\sum_{i=1}^n(\hat{y}_{i} - \bar{y})^2 = (n - 1) \cdot Var(\hat{y})$<span style="float:right;">[2.11]</span></center>

<br><center>Residual sum of the squares (SSR) = $\sum_{i=1}^n(\hat{\mu}_{i} - 0)^2 = (n - 1) \cdot Var(\hat{\mu})$<span style="float:right;">[2.12]</span></center><br>
...
```{r}
n <- length(ceosal1$salary)
SST <- (n - 1) * stats::var(ceosal1$salary)
SSE <- (n - 1) * stats::var(fitted(roe_salary_lm))
SSR <- (n - 1) * stats::var(resid(roe_salary_lm))
```
$SST = (n - 1) \cdot Var(y)$ = `r SST`

$SSE = (n - 1) \cdot Var(\hat{y})$ = `r SSE`

$SSR = (n - 1) \cdot Var(\hat{\mu})$ = `r SSR`
...
```{r}
R_2_3 <- stats::cor(ceosal1$salary, fitted(roe_salary_lm))^2
```
`stats::cor()` = `r R_2_3`
...
```
## 2.4 Nonlinearities
According to `(Wooldridge, page 37)` "...it is rather easy to incorporate many nonlinearities into simple regression analysis by appropriately defining the dependent and independent variables" (i.e. a variable transformation).
...
<center>$log(wage) = \beta_{0} - \beta_{1}educ + \mu$<span style="float:right;">[2.10]</span></center>
<div class="task">Task: Using equation [1] estimate the linear model of `log(wage)` (i.e. the natural log of *wage*) regressed on *educ*.</div>  
```{r}
log_wage_educ_lm <- stats::lm(log(wage) ~ educ, data = wage1)
...
<div class="task">Task: Exponentiate equation [1] and plot the nonlinear nature of how *wage* increases for every additional year of education.</div> 

1. By exponentiating [1], we can write an equation involving just *wage* and *educ*: $$\widehat{wage} = exp(\hat{\beta_{0}} + \hat{\beta_{1}}educ + \hat{\mu)}$$
```{r}
educ <- seq(0, 19, 1)
log_wage <- b0 + b1 * educ
wage <- exp(log_wage)
...
RplotterPkg::create_scatter_plot(
  ...
  aes_y = "wage",
  title = "Wage Regressed on Education",
  subtitle = "wage = exp(0.59 + 0.083*educ)",
  connect = TRUE,
  x_limits = c(0,19),
...
<div class="task">Task: Using the above data of *salary* on *roe* compute the model $\hat{y} = \hat{\beta}_{1} \cdot x$ (i.e. regression is through the origin).</div>  
```{r}
roe_salary_no_intercept_lm <- stats::lm(formula = salary ~ 0 + roe, data = ceosal1)
```

 $\hat{\beta}_{1} =$ `r coef(roe_salary_no_intercept_lm)[[1]]`

<div class="task">Task: Using the above data of *salary* on *roe* compute the model $\hat{y} = \hat{\beta}_{0}$ (i.e. the regression is a constant).</div> 
```{r}
roe_salary_constant_lm <- stats::lm(formula = salary ~ 1, data = ceosal1)
```

 $\hat{\beta}_{0} =$ `r coef(roe_salary_constant_lm)[[1]]`

<div class="task">Task: Plot all three regression models of *salary* on *roe*.</div>

1. Create a data frame with *roe* and the three fitted values of *salary*:
```{r}
df_obser <- data.frame(
  roe = ceosal1$roe,
  salary = roe_salary_lm$model$salary,
...
df_no_intercept <- data.frame(
  roe = ceosal1$roe,
  salary = roe_salary_no_intercept_lm$fitted.values,
  type = "No Intercept"
)
df_constant <- data.frame(
  roe = ceosal1$roe,
  salary = roe_salary_constant_lm$fitted.values,
  type = "Constant"
)
```
...
2. Plot the data:
```{r}
df <- rbind(df_obser,df_no_intercept,df_constant)
RplotterPkg::create_scatter_plot(
  df = df,
  aes_x = "roe",
  aes_y = "salary",
...
<center>$se(\hat{\beta}_{0}) = \frac{1}{\sqrt{n-1}}\cdot\frac{\hat{\sigma}}{sd(x)}\cdot\sqrt{\bar{x^2}}$<span style="float:right;">[2.15]</span></center>

<center>$se(\hat{\beta}_{1}) = \frac{1}{\sqrt{n-1}}\cdot\frac{\hat{\sigma}}{sd(x)}$<span style="float:right;">[2.16]</span></center>
...
<div class="task">Task: Using the data set `Wooldridge::meap93`, compute the standard errors of the OLS estimators using the above equations.</div>  
...
1. Review the *lnchprg* and *math10* variables from the data set:
```{r}
...
```{r}
math_lunch_lm <- stats::lm(formula = math10 ~ lnchprg, data = meap93)
```

3. Compute the **residual standard error** (SER):
```{r}
n = stats::nobs(math_lunch_lm)
...
```{r}
beta_0_se <- (SER/sd(meap93$lnchprg))/sqrt(n-1) * sqrt(mean(meap93$lnchprg^2))
beta_1_se <- (SER/sd(meap93$lnchprg))/sqrt(n-1)
```
...
### 2.7.1 One sample
<div class="task">Task: We are given population coefficients for $\beta_{0}$ = 1, $\beta_{1}$ = 0.5, and set the sd of the error term $\mu$ to $\sigma$ = 2.  If we draw a random sample ($n$ = 1000) of the independent variable $x$, then the population $y$ values can be computed from the population coefficients. If we estimate the coefficients via OLS, using $x$ and $y$ as input, how does the estimated $\hat{y}$ compare with the population $y$ values.</div>  
...
RplotterPkg::create_scatter_plot(
  ...
  aes_x = "x",
  aes_y = "y",
  title = "Simulated sample and OLS Regression Line",
  rot_y_tic_label = TRUE
) + geom_line(data = df_ols, aes(x = x, y = y), color = "blue")
```

<div class="task">Task: Compute the standard errors of the above $\hat{\beta_{0}}$ and $\hat{\beta_{1}}$ OLS estimates.</div>  
...
<div class="task">Task: Repeat the simulation in the above section by computing a random set of $x$ values along with 10000 random normal sets of the error term $\mu$</div> 
...
ols_plot <- RplotterPkg::create_scatter_plot(
 ...
 aes_x = "x",
 aes_y = "y",
 title = "Simulated sample and OLS Regression Lines",
 subtitle = "OLS regressions(gray); population regression(red)",
 connect = T,
 rot_y_tic_label = T,
 show_pts = F,
 line_color = "red"
...
<div class="task">Task: Repeat the many sample Monte Carlo simulation in the above section 2.7.2 and compute a randomly selected population $\mu$ where the $\mu$'s mean is conditional on $x$. For example set $E(\mu_{i}|x_{i}) = \frac{x_{i}-4}{5}$</div>  
...
<div class="task">Task: As an example of the SLR.5 violation, repeat the many sample Monte Carlo simulation in the above section 2.7.2 and set $Var(\mu_{i}|x_{i}) = \frac{4}{e^{4.5}}\cdot e^{x_{i}}$</div> 
