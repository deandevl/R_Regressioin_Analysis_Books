...
<div class="task">Task: Apply the `stats::lm()` function to determinants of College GPA `(Wooldridge, example 3.1, page 66)`.</div>  
...
2. Apply `stats::lm()`:
```{r}
gpa_model <- stats::lm(formula = colGPA ~ hsGPA + ACT, data = gpa_1)
summary(gpa_model)
```

<div class="task">Task: Apply the `stats::lm()` function to determinants of hourly wage `(Wooldridge, example 3.2, page 67)`.</div> 

1. Create a data.table:
```{r}
wage_1 <- data.table::setDT(wooldridge::wage1)
str(wage_1)
```

2. Apply `stats::lm()`:
```{r}
wage_model <- stats::lm(formula = log(wage) ~ educ + exper + tenure, data = wage_1)
summary(wage_model)
```
<div class="task">Task: Repeat the estimate of *wage_model* with just the *educ* variable.</div>    
```{r}
wage_model_educ <- stats::lm(formula = log(wage) ~ educ, data = wage_1)
summary(wage_model_educ)
```


<div class="task">Task: Apply the `stats::lm()` function to determinants of 401(k) participation rate `(Wooldridge, example 3.3, page 70)`.</div> 

1. Create a data.table:
```{r}
k_401k <- data.table::setDT(wooldridge::k401k)
str(k_401k)
```

2. Apply `stats::lm()`:
```{r}
k401k_model <- stats::lm(formula = prate ~ mrate + age, data = k_401k)
summary(k401k_model)
```
<div class="task">Task: Apply the `stats::lm()` function to determinants of arrest `(Wooldridge, example 3.5, page 72)`.</div>   

1. Create a data.table:
```{r}
crime_1 <- data.table::setDT(wooldridge::crime1)
str(crime_1)
```

2. Apply `stats::lm()`:
```{r}
crime_model <- stats::lm(formula = narr86 ~ pcnv + ptime86 + qemp86, data = crime_1)
summary(crime_model)
```
<div class="task">Task: Repeat the estimate of *crime_model* with *avgsen* (average sentence) variable.</div>   
```{r}
crime_model_avgsen <- stats::lm(formula = narr86 ~ pcnv + ptime86 + qemp86 + avgsen, data = crime_1)
...
<center>$\hat{\beta} = (X^\prime X)^{-1}X^\prime y$<span style="float:right;">[3.2]</span></center>

The vector of residuals can be manually calculated as:
<center>$\hat\mu = y - X\hat\beta$<span style="float:right;">[3.3]</span></center>

The estimated variance of the error term $\hat\sigma^2$ is:
<center>$\hat\sigma^2 = \frac{1}{n-k-1}\hat\mu^\prime \hat\mu$<span style="float:right;">[3.4]</span></center>
...
<center>$\widehat{Var(\hat\beta)}=\hat\sigma^2(X^\prime X)^{-1}$<span style="float:right;">[3.5]</span></center>
...
k <- 2
n <- nrow(gpa_1)
X <- cbind(1, gpa_1$hsGPA, gpa_1$ACT)
Y <- gpa_1$colGPA
head(X)
```
...
<div class="task">Task: Using the college GPA data `(wooldridge::gpa1)`, repeat the regression of college GPA(*colGPA*) on high school GPA(*hsGPA*) and ACT achivement test(*ACT*).  Study the *ceteris paribus* effect of ACT on college GPA.</div>    
...
```{r}
gpa_model <- lm(colGPA ~ ACT + hsGPA, data = gpa_1)
...
```{r}
ACT_hsGPA_coef <- coef(lm(hsGPA ~ ACT, data = gpa_1))["ACT"]
```
...
```{r}
ACT_coef <- coef(lm(colGPA ~ ACT, data = gpa_1))["ACT"]
```
...
```{r}
hsGPA_ACT_model <- lm(hsGPA ~ ACT, data = gpa_1)
...
```{r}
n <- nobs(gpa_model)
hsGPA_se <- 1/sqrt(n-1) * SER/sd(gpa_1$hsGPA) * sqrt(hsGPA_VIF)
```

<div class="task">Task: Using the data set `wooldridge::wage1` for determining hourly wage `(Wooldridge, example 3.2, page 67)`, compute and compare the regressors VIF values as computed from the `car::vif()`.</div>    
...
2. Regress *log(wage)* on *educ*, *exper*, *tenure*:
```{r}
wage_model <- lm(log(wage) ~ educ + exper + tenure, data = wage_1)
summary(wage_model)
```

3. Automatically calculate VIF of the regressors:
```{r}
car::vif(wage_model)
```
