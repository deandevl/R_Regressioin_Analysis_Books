...
<center>$\hat{y_{i}} = \hat{\beta_{0}} + \hat{\beta_{1}}x_{i}$<span style="float:right;">[2.20 Wooldridge page 27]</span></center><br>
where $\hat{y_{i}}$ is the **fitted value** for $y$ at observation $i$.
...
<center>$\hat{\beta_{1}} = \frac{Cov(x,y)}{Var(x)}$<span style="float:right;">[2.3 Heiss page 69]</span></center><br>

<div class="task">Task:  From the `wooldridge::ceosal1` data set involving "CEO Salary" (*salary*) and "Return on Equity" (*roe*) calculate the statistics for estimating the population parameters $\beta_{0}$ and $\beta_{1}$ for the *roe* variable to estimate *salary*.</div>
...
ceosal1_dt <- data.table::setDT(ceosal1)
ceosal1_dt <- ceosal1_dt[, .(roe, salary)]
```
...
where $\hat{\rho}_{xy}$ is the simple correlation between $x_{i}$ and $y_{i}$ and $\hat{\sigma}_{y}$ and $\hat{\sigma}_{x}$ are the sample standard deviations.
Wooldridge brings up an important point in regard to this estimate:

<blockquote>Recognition that $\beta_{i}$ is just a scaled version , $\rho_{xy}$, highlights an important limitation of simple regression when we do not have experimental data: In effect, simple regression is an analysis of correlation between two variables, and so one must be careful in inferring causality.</blockquote>

Illustrating this second form:
```{r}
...
<div class="takeaway">Take Away: Both forms of estimating $\hat{\beta_{1}}$ are equal and is a scaled version of the correlation between $x_{i}$ and $y_{i}$.</div><br>
...
<center>$\hat{\mu_{i}} = y_{i} - \hat{y_{i}} = y_{i} - \hat{\beta_{0}} + \hat{\beta_{1}}x_{i}$<span style="float:right;">[2.21 Wooldridge page 27]</span></center><br>

or [Heiss] rearranges to give:
<center>$y_{i} = \hat{\beta_{0}} + \hat{\beta_{1}}x + \hat{\mu_{i}}$<span style="float:right;">[2.1 Heiss page 69]</span></center><br> 

We also have the population model for $y_{i}$:
<center>$y_{i} = \beta_{0} + \beta_{1}x + \mu_{i}$<span style="float:right;">[2.48 Wooldridge page 41]</span></center><br>
...
<center>$\hat{\mu_{i}} = \mu_{i} - (\hat{\beta_{0}} - \beta{_{0}}) - (\hat{\beta_{1}} - \beta_{1})x_{i}$<span style="float:right;">[2.59 Wooldridge page 49]</span></center><br>
...
<center>$\sum_{i=1}^n \hat{\mu_{i}} = 0 \Rightarrow \bar{\hat{\mu}}_{i} = 0$<span style="float:right;">[2.7 Heiss page 76]</span></center><br>
...
<div class="task">Task: Using equation [2.42] estimate the linear model of `log(wage)` (i.e. the natural log of *wage*) regressed on *educ*.</div>  
```{r}
...
<div class="task">Task: Exponentiate equation [2.42] and plot the nonlinear nature of how *wage* increases for every additional year of education.</div> 
