...
gpa2_dt <- gpa2_dt[, .(colgpa = colgpa, sat = sat - 1200, hsperc = hsperc - 30, hsize = hsize - 5)]
head(gpa2_dt)
```
...
CI_upper <- gpa2_ols_new_lst$coef[["(Intercept)"]] + gpa2_ols_new_lst$coef_se[["(Intercept)"]] * t_val
CI_lower <- gpa2_ols_new_lst$coef[["(Intercept)"]] - gpa2_ols_new_lst$coef_se[["(Intercept)"]] * t_val
```
...
<div class="takeaway">Take Away: The two approaches agree.</div>
