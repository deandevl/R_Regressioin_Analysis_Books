...
<div class="takeaway">Take Away: Under CLM, the OLS estimators are normally distributed</div>
...
<div class="takeaway">Take Away: The $t$ distribution (with n-k-1 degrees of freedom) allows us to make inferences/hypothesis' on the population $\beta_{j}$</div>
...
<div class="takeaway">Take Away: The critical values are nearly the same.</div>
...
RplotterPkg::create_table(
  x = dt,
  caption = "Predictor values, $se$, $t$, $p$, statistics",
  col_names = c("Predictor", "$\\beta_{j}$", "$se(\\hat{\\beta}_{j})$", "$t(\\hat{\\beta}_{j})$", "$p(\\hat{\\beta}_{j})$")
)
```

<div class="takeaway">Take Away: The $t$ values for all the coefficients except $\hat{\beta}_{act}$ are larger in absolute value than the critical $t$ values in **Table 4.1 Critical $t$ Values** above. So we would reject $H_{0}$ for all the usual significance levels.</div>


### 4.1.3 Other hypotheses

We are skipping Heiss' "Hourly Wage Equation" page 107 and going to Wooldridge's Example 4.4 page 116 "Campus Crime and Enrollment".</div>
...
1.  Read in the data:
```{r}
crime_dt <- data.table::setDT(wooldridge::campus)
crime_dt <- crime_dt[, .(crime, enroll)]
```
...
<div class="takeaway">Take Away: With the $t$ statistic for $\hat{\beta}_{enroll}$ greater than the critical $t$ value, we reject the null hypothesis $H_{0}: \beta_{enroll} = 1$</div>
...
1.  Read in the data:
```{r}
rdchem_dt <- data.table::setDT(wooldridge::rdchem)
...
twoyear_dt <- twoyear_dt[, .(lwage, jc, univ, exper)]
```
...
```{r}
diff_jc_univ <- as.numeric(twoyear_ols_lst$coef["jc"] - twoyear_ols_lst$coef["univ"])
diff_jc_univ
```
...
<div class="takeaway">Take Away: As Wooldridge later comments on page 126, "there is some, but not strong, evidence against Equation [4.18] above."</div>
...
RplotterPkg::create_table(
  x = new_ols$coef_df,
  caption = "lwage ~ jc + totcoll + exper"
)
```
...
```{r}
theta <- new_ols$coef[["jc"]]
theta_se <- new_ols$coef_se[["jc"]]
t_theta <- theta/theta_se
```

<div class="takeaway">Take Away: Comparing *t_jc_unit* in `Alternative 1` above with *t_theta* in this `Alternative 2`, they are the same.</div>
...
<div class="takeaway">Take Away: It appears that the estimate $t$ value does not go beyond the critical value so we do not reject the null hypothesis $H_{0}: \beta_{1} = \beta_{2}$.</div>

<div class="takeaway">Take Away: Single hypothesis tests concerning more than one $\beta_{j}$ can always be tested by rewriting the model to contain the parameter of interest. Then, a standard $t$ statistic can be used.</div>
...
```{r}
c <- stats::qf(1 - 0.01, q, n - k - 1)
c
```

With *F_val* greater than *c* we reject the above null hypothesis and believe that at least one of the performance has an influence on salary.
...
The function `RregressPkg::ols_F_calc()` provides a easy way to compute the $F$ value:
```{r}
...
RplotterPkg::create_table(
  x = mlb1_F_df,
  caption = "Null Hypothesis: bavg = hrunsyr = rbisyr = 0"
)
```
...
hprice1_dt <- hprice1_dt[, .(price = log(price), assess = log(assess), lotsize = log(lotsize), sqrft = log(sqrft), bdrms)]
```
...
RplotterPkg::create_table(ur_ols$coef_df, caption = "price ~ assess + lotsize + sqrft + bdrms")
```
...
RplotterPkg::create_table(r_ols$coef_df, caption = "price-assess ~ (Intercept)")
```
...
With *F_val* less than *c* we do not reject the null hypothesis.
